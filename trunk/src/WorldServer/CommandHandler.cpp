...
{	
	...
	else if(command.substr(1, 8) == "teleport")
	{
		...
		teleportChar(map, GTable, clientList);
	}
...
void WorldServer::teleportChar(string zone, GlobalTable* GTable, vector<GameClient*>* clientList)
{
	...
	{
		GTable->client->charInfo.map = map.mapID;
		GTable->client->charInfo.position.x = map.position.x;
		GTable->client->charInfo.position.y = map.position.y;
		GTable->client->charInfo.position.z = map.position.z;
		GTable->client->charInfo.rotation.x = map.rotation.x;
		GTable->client->charInfo.rotation.y = map.rotation.y;
		GTable->client->charInfo.rotation.z = map.rotation.z;
		//Save new position
		Database.saveCharPosition(GTable->client);
		...
		aBuffer.writeHeader(sender, sizeof(sender), receiver, sizeof(receiver), 0x2000, true);
		size = sizeof(pack2_0) + sizeof(pack2_1) + (10 * 4) + 1;
		aBuffer.write<uint32>(size);
		...
		aBuffer.write<uint32>(GTable->client->charInfo.position.x);
		aBuffer.write<uint32>(GTable->client->charInfo.position.y);
		aBuffer.write<uint32>(GTable->client->charInfo.position.z);
		aBuffer.write<uint32>(0x00000032);
		aBuffer.write<uint8>(0);
		aBuffer.doItAll(GTable->client->clientSocket);
		//*/
		/*
		uint32 i_nDimID = (GTable->client->nClientInst >> 24) & 0x000000ff;
		RealmInfo* realm = 0;
