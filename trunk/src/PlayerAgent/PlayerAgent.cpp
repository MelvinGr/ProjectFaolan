...
void PlayerAgent::HandleClient(void* socket)
{
	...
	while(client->isConnected)
	{
		...
		if(packet)
		{
			PlayerAgentHandler(packet, client);
			delete packet;
		}
	}
...
{
#ifdef WINDOWS
	SetConsoleTitle("Project Faolan - PlayerAgent");
#endif

	Log.Start("PlayerAgent.txt");
...
#ifdef WINDOWS
			CreateThread(0, 0, (LPTHREAD_START_ROUTINE)PlayerAgent::HandleClient, &client, 0, 0);
#else
			pthread_create(0, 0, (void*(*)(void*))PlayerAgent::HandleClient, &client);
#endif
		}
	}
