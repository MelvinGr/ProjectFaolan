...
namespace Faolan.Core.Data
{
    public sealed class Character
    {
        ...
        {
            /*var c = database.ExecuteReader($"SELECT * FROM characters WHERE character_id={Id}").ToDictionary();
            if (c.Count == 0)
        ...
        {
            return false; /*database.ExecuteNonQuery($"UPDATE characters SET level={Level}, map_id={Map}," + 
                $"pos_x={Position.X}, pos_y={Position.Y}, pos_z={Position.Z}," +
                $"rot_x={Rotation.X}, rot_y={Rotation.Y}, rot_z={Rotation.Z} WHERE character_id={Id}") == 1;*/
        }
